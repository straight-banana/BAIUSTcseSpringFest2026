...
import { getRoster, setCaptain as setCaptainApi } from '../../services/candidatesService.js';
...
export default function AssignCaptain() {
  ...
  useEffect(() => {
    ...
    getRoster()
      ...
      .catch((err) => { if (active) setError(err?.message || 'Unable to load roster'); })
      .finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, []);

  const classOptions = useMemo(() => [...new Set(students.map((s) => s.className).filter((c) => c != null))].sort((a, b) => a - b), [students]);
  const sectionOptions = useMemo(() => [...new Set(students.map((s) => s.section).filter(Boolean))].sort(), [students]);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    return students.filter((s) => {
      if (cls !== 'all' && String(s.className) !== cls) return false;
  ...
  const totalSections = classOptions.length * sectionOptions.length;

  const handleToggle = async (student) => {
    const next = !student.isCaptain;
    try {
      await setCaptainApi(student.id, next);
      setStudents((prev) => prev.map((s) => {
        if (s.id === student.id) return { ...s, isCaptain: next };
        if (next && s.className === student.className && s.section === student.section) return { ...s, isCaptain: false };
        return s;
      }));
      toast.push?.({
      ...
      toast.push?.({ title: 'Action failed', message: err?.message || '', tone: 'error' });
    }
  };
  ...
  return (
    <PageContainer>
      ...
      {loading ? (
        ...
        <ErrorState title="Couldn't load roster" message={error} />
      ) : (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            ...
            <StatTile icon={<GraduationCap size={16} />} label="Classes" value={classOptions.length} />
            <StatTile icon={<Users size={16} />} label="Sections / class" value={sectionOptions.length} />
          </div>
          ...
          <Card className="p-4 mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              ...
              <SelectBox label="Class" value={cls} onChange={setCls} options={[['all','All classes'], ...classOptions.map((c)=>[String(c),`Class ${c}`])]} />
              <SelectBox label="Section" value={sec} onChange={setSec} options={[['all','All sections'], ...sectionOptions.map((s)=>[s,`Section ${s}`])]} />
            </div>
          </Card>
