...
export default function CandidateCompare() {
  ...
  useEffect(() => {
    ...
    getActive()
      .then((e) => {
        ...
        setCandidates(list);
        setIds(list.slice(0, 2).map((c) => c.id));
      })
      .catch((err) => { if (active && err?.status !== 404) setError(err?.message || 'Unable to load candidates'); })
  ...
  if (loading) {
    return (
      <PageContainer>
        <PageHeader title="Compare Candidates" subtitle="Line up 2–4 candidates side by side." icon={<GitCompare size={18} />} />
        <Mission9SubNav />
          ...
          <div className={cx('grid gap-4 mb-6',
            ...
            {selected.map((s, i) => (
              <Card key={s.id} className="p-4 relative">
                ...
                <p className="mt-3 text-xs text-muted line-clamp-3 min-h-[3rem]">{s.bio || 'No bio provided.'}</p>
              </Card>
            ))}
          </div>

          <Card className="p-4 overflow-x-auto">
            <SectionHeader title="Bio highlights" description="Side-by-side bios" />
              ...
              <tbody>
                {selected.map((s, i) => (
                  <tr key={s.id} className="border-t border-border align-top">
                    ...
                    <td className="px-3 py-3 text-muted italic">{s.bio ? `"${s.bio}"` : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
